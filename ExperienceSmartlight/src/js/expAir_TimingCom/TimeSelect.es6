...
export const TimeSelect = React.createClass({
	...
	submitclock:function(e){
		...
		let control = this.props.control?this.props.control:'';
		//确认提交时间
		...
		let TransmitValue = this.state.hourarr;   //传递进来的值
		let getValue = this.state.hourtime;       //选择的值
		let index;
		let getMinuteValue = this.state.minutetime;

		//点击模式,直接是第一个模式'自然风'--确定 传过来的是0,滑动后选第一个模式,传过来就是自然风。
		if(getValue==0 && getMinuteValue==0){
			if(control==1){    getValue='自动';}
			if(control==2){    getValue='大';}
			if(control==3){    getValue='上下';}

			//当模式--第一个模式是0时,定时的状态原本是什么状态就什么状态。
			if(this.props.judgeClock==1 ){
				Actions.judgeClock(1);
			}else if(this.props.judgeClock==3){
				Actions.judgeClock(3);
			}else{
				Actions.judgeClock(2);
			}
		...
		if(typeof(getValue) =="string"){
			for(var i=0;i<TransmitValue.length;i++){
				if(TransmitValue[i]==getValue){
					index = i+1;
					Actions.selectMode(control,index);
	...
	render: function() {
		...
		let deviceClick=this.props.deviceClick?this.props.deviceClick:0;
		return (
